...
export function customTimeAxis(scaleInput) {
  ...
  function choosePrimaryLevel(domainSpanMonths) {
    if (domainSpanMonths <= 12) return TIME_LEVELS.month;
    if (domainSpanMonths <= 36) return TIME_LEVELS.quarter;
    return TIME_LEVELS.year;
  }
  ...
  function isMajorTick(d, level) {
    ...
    const parent = TIME_LEVELS[level.parent];
    return parent.interval.floor(d).getTime() === d.getTime();
  }


  // function midpointLabels(ticks, start, end) {
  //   const labels = [];

  //   for (let i = 0; i < ticks.length - 1; i++) {
  //     labels.push(new Date((ticks[i].getTime() + ticks[i + 1].getTime()) / 2));
  //   }

  //   if (ticks.length) {
  //     if (ticks[0] > start)
  //       labels.unshift(new Date((start.getTime() + ticks[0].getTime()) / 2));

  //     if (ticks[ticks.length - 1] < end)
  //       labels.push(new Date((ticks[ticks.length - 1].getTime() + end.getTime()) / 2));
  //   }

  //   return labels;
  // }

  function spansParentUnit(ticks, parentLevel) {
    if (!parentLevel) return false;
    const values = new Set(ticks.map(d => parentLevel.key(d)));
    return values.size > 1;
  }

  function renderLabelLayer(g, scale, labels, format, offset, levelName) {
    ...
    const layer = g.append("g")
      .attr("class", `axis-label-layer axis-label-${levelName}`)
  ...
  function axis(selection) {
    selection.each(function () {
      ...
      const primaryLevel = choosePrimaryLevel(domainSpanMonths);
      ...
      // -----------------------------
      // Label layers (hierarchical)
      // -----------------------------
      let currentLevel = primaryLevel;
      let currentTicks = ticks;
      let yOffset = tickPadding;


      while (currentLevel) {
        // ---- Primary level (fine labels)
        if (currentLevel === primaryLevel) {
          const primaryRangesData = primaryRanges(currentTicks, start, end);
          ...
          // const labels = midpointLabels(currentTicks, start, end);
          const format =
            userTickFormat
              ? typeof userTickFormat === "function"
                ? userTickFormat
                : d3.utcFormat(userTickFormat)
              : currentLevel.format;

          // renderLabelLayer(g, scale, labels, format, yOffset, currentLevel.name);

          const primaryLayer = renderLabelLayer(
            g,
            scale,
        ...
        // ---- Parent level (range labels)
        const parentName = currentLevel.parent;
        const parentLevel = parentName ? TIME_LEVELS[parentName] : null;

        if (!spansParentUnit(currentTicks, parentLevel)) break;
        ...
        const visibleParentRanges = filterByAvailableSpace(
          parentRangesData,
          scale,
          minLabelWidth * 1.2 // parents usually need a bit more room
        );

        if (visibleParentRanges.length && config.labelSpans.secondaryLabels) {
          yOffset += 16;

          const parentLayer = renderLabelLayer(
            g,
            scale,
            parentRangesData.map(d => d.mid),
            parentLevel.format,
            yOffset,
            parentLevel.name
          );
