...
export function setupAnnotationToolbar() {
  const parent = d3.select("body");
  // Check if the div already exists within the body.
  if (parent.select("div#toolbar").empty()) {
    // If it doesn't exist, insert a new div and add the class.
    parent.insert("div", "#graphic")
      .attr("id", "toolbar");
  } else {
    parent.select("div#toolbar").selectAll("*").remove()
  }
}
...
export function createAnnotationToolbar(wholeSvg, chartContainer, scales, margin, chart_width, chart_height) {
  ...
  const annotationsNames = [
    {
      ...
      icon: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-move-horizontal-icon lucide-move-horizontal"><path d="m18 8 4 4-4 4"/><path d="M2 12h20"/><path d="m6 8-4 4 4 4"/></svg>'
    },
    {
      ...
      icon: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-move-vertical-icon lucide-move-vertical"><path d="M12 2v20"/><path d="m8 18 4 4 4-4"/><path d="m8 6 4-4 4 4"/></svg>'
    }
  ]

  const fieldset = d3.select("#toolbar").append("fieldset")
  ...
  const button = d3.select("#toolbar")
    .append("button")
    .attr("id", "clicktoadd")
    .attr('class','ons-btn')
    .on("click", function () {
