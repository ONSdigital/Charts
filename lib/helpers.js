...
/**
 * Calculate Y-axis bounds based on data and config settings.
 * Supports "auto" mode (smart trimming with zero baseline preference),
 * "data" mode (exact data bounds), or custom numeric values.
 * @param {Array} graphicData - Array of data objects with date and category values.
 * @param {Object} config - Configuration object with yDomainMin and yDomainMax settings.
 * @returns {Object} Object with minY and maxY properties.
 */
export function calculateAutoBounds(graphicData, config) {
  // Get categories from the data, excluding date and series columns (includes CI columns)
  const categories = Object.keys(graphicData[0]).filter((k) =>
    k !== 'date' &&
    k !== 'series' &&
    k !== 'name'
  );
  ...
  // Handle "data" mode - use exact data bounds without any calculations
  if (config.yDomainMin === "data" && config.yDomainMax === "data") {
    return { minY: dataYMin, maxY: dataYMax };
  }

  const dataRange = dataYMax - dataYMin;

  // Helper function to calculate a single auto bound with smart trimming
  const calculateSingleBound = (dataValue, oppositeBound, isMin) => {
    // Start at zero, or use data value with 15% of data range as padding if beyond zero
  ...
  if (config.yDomainMax === "auto") {
    maxY = calculateSingleBound(dataYMax, dataYMin, false);
  } else if (config.yDomainMax === "data") {
    maxY = dataYMax;
  } else {
  ...
  if (config.yDomainMin === "auto") {
    minY = calculateSingleBound(dataYMin, maxY, true);
  ...
  // Validate: ensure maxY is greater than minY
  if (minY >= maxY) {
    console.warn(`calculateAutoBounds: minY (${minY}) >= maxY (${maxY}), swapping values`);
    return { minY: Math.min(minY, maxY), maxY: Math.max(minY, maxY) };
  }

  return { minY, maxY };
}

export function initialise(size, config = {}, overrides = {}) {
  ...
  // Set the visually-hidden accessible summary from the chart's config
  if (config.accessibleSummary) {
...
export function addDataLabels({
  ...
  const x0 = xScaleFunction(0);
  const threshold = chartWidth / labelPositionFactor;

  const isSmall = d =>
    Math.abs(xScaleFunction(d.value) - x0) < threshold;
  ...
  svgContainer
    ...
    .attr('y', d =>
      y2function
        ? yScaleFunction(d.name) + y2function(d.category) + y2function.bandwidth() / 2
    ...
    .attr('fill', (d) => {
      if (isSmall(d)) {
        ...
        // Label is inside the bar - determine bar color
        let barColor;
        if (colourScale) {
          if (typeof colourScale === 'function') {
            barColor = d.category ? colourScale(d.category) : colourScale(d.name);
          } else {
            barColor = colourScale;
          }
        }

        if (barColor) {
          return getTextColorFromBackground(barColor);
        }
        return "#ffffff"; // Fallback to white if no color determined
      }
    })
     .attr('stroke', (d) => {
      if (isSmall(d)) {
        ...
        return 'none'; // No stroke for labels inside the bar
      }
    })
    .text((d) =>
...
export function getDataLabelX({
  ...
  const threshold = chartWidth / labelPositionFactor;
  const isSmall =
    Math.abs(xScaleFunction(datum.value) - x0) < threshold;
  const padding = 4;
  const barStart = Math.min(x0, xScaleFunction(datum.value));
  const barEnd = Math.max(x0, xScaleFunction(datum.value));

  return isSmall
    ? (datum.value > 0 ? barEnd + padding : barStart - padding)
    : (datum.value > 0 ? barEnd - padding : barStart + padding);
}
  ...
  svgContainer
    ...
    .attr("fill", (d) => {
      ...
      if (columnHeight < minColumnLabelHeight) {
        ...
        // Label is inside the column - determine bar color
        let barColor;
        if (colourScale) {
          if (typeof colourScale === 'function') {
            barColor = d.category ? colourScale(d.category) : colourScale(d.name);
...
export function getIndexedShape(index) {
  const shapeIndex = index % 6;
  const isFilled = shapeIndex < 3;
  const shapeType = shapeIndex % 3;
  const shape = shapeType === 0 ? "circle" : shapeType === 1 ? "square" : "diamond";

  return { shape, isFilled };
}

export function drawShapeMarker({
  svg,
  shape,
  color,
  x,
  ...
  const fill = isFilled ? color : "white";
  const resolvedDiamondSize = diamondSize ?? size * 1.75;
  ...
  group
    .append("path")
    .attr("d", diamondShape(resolvedDiamondSize))
    .style("fill", fill)
    .style("stroke", color);
}

export function drawIndexedLegendShape({
  svg,
  index,
  color,
  size = 4,
  ...
  if (useLines) {
    // Draw a line instead of a shape marker
    svg
      .append("line")
      .attr("x1", 0)
      .attr("y1", 6)
    ...
    const { shape, isFilled } = getIndexedShape(index);
...
export function drawIndexedLineEndMarker({
  svg,
  index,
  color,
  x,
...
export function wrapStyled(text, width, lineHeightEms = 1.1) {
  text.each(function () {
    ...
    // Extract inline tspans (styled parts)
    const parts = [];
    textSel.selectAll("tspan").each(function () {
      const t = d3.select(this);
      parts.push({
        text: t.text(),
        fontWeight: t.attr("font-weight") || "400",
        dx: t.attr("dx")
      });
    });

    textSel.text(null); // clear everything

    let lineTspan = textSel.append("tspan").attr("dy", "0.35em");
    lineCount++
    ...
    parts.forEach(part => {
      ...
      words.forEach((word, i) => {
        const spacer = i === 0 ? "" : " ";
        const testTspan = lineTspan.append("tspan")
          .attr("font-weight", part.fontWeight)
          .attr('dx', i == 0 ? part.dx : 0)
          .text(spacer + word);

        if (lineTspan.node().getComputedTextLength() > width) {
          testTspan.remove();

          lineTspan = textSel.append("tspan")
            .attr("x", x)
            .attr("dy", lineHeightEms + "em");
          lineCount++
    ...
    textSel.attr("data-line-count", lineCount);
  });
}
...
export function addAnnotationText(
  svgName,
  xValue,
  yValue, TextOffsetX, TextOffsetY, thisText, textAnchor, wrapWidth, mobileText, mobileTextLetter, MobileCircleOffsetX, MobileCircleOffsetY, size, editable = false, id) {
...
export function addAnnotationArrow(svgName, xValue, yValue, arrowOffsetX, arrowOffsetY, xLength, yLength, curve, thisText, textPosition, wrapWidth, mobileText, mobileTextLetter, MobileCircleOffsetX, MobileCircleOffsetY, size, editable = false, id) {
 ...
 * @param {string} mobileTextLetter - Identifier for mobile text version.
 * @param {number} MobileCircleOffsetX - Horizontal offset for mobile circle.
 * @param {number} MobileCircleOffsetY - Vertical offset for mobile circle.
 ...
 */
export function addAnnotationLineVertical(svgName, height, xValue, thisText, textPosition = 'right', yValue, wrapWidth, mobileText, mobileTextLetter, MobileCircleOffsetX, MobileCircleOffsetY, moveToBack, size, editable = false, id) {
 ...
 */
export function addAnnotationLineHorizontal(svgName, width, yValue, thisText, textPosition = "below", xValue, wrapWidth, mobileText, mobileTextLetter, MobileCircleOffsetX, MobileCircleOffsetY, moveToBack, size, editable = false, id) {
 ...
 * @param {string} size - Chart size ('sm', 'md', 'lg').
 */
export function addAnnotationRangeVertical(svgName, height, xValue, xEndValue, thisText, textPosition = "left", textPosition2 = "inside", yValue = 10, wrapWidth, mobileText, mobileTextLetter, MobileCircleOffsetX, MobileCircleOffsetY, size, editable = false, id) {
 ...
 * @param {string} size - Chart size ('sm', 'md', 'lg').
 */
export function addAnnotationRangeHorizontal(svgName, width, yValue, yEndValue, thisText, textPosition = 'above', textPosition2 = 'inside', xValue, wrapWidth, mobileText, mobileTextLetter, MobileCircleOffsetX, MobileCircleOffsetY = 10, size, editable = false, id) {
 ...
 * @param {string|number} [config.mobile.letter] - Mobile text letter
  ...
  switch (type) {
    case 'arrow':
      return addAnnotationArrow(
        ...
        mob.letter, mob.circleOffsetX, mob.circleOffsetY, sty.size, editable, id
      );
    ...
    case 'text':
      return addAnnotationText(
        ...
        txt.wrapWidth, mob.enabled, mob.letter, mob.circleOffsetX,
    ...
    case 'line-vertical':
      return addAnnotationLineVertical(
        ...
        mob.enabled, mob.letter, mob.circleOffsetX, mob.circleOffsetY,
    ...
    case 'range-vertical':
      return addAnnotationRangeVertical(
        ...
        y || 10, txt.wrapWidth, mob.enabled, mob.letter,
    ...
    case 'range-horizontal':
      return addAnnotationRangeHorizontal(
        ...
        x || 10, txt.wrapWidth, mob.enabled, mob.letter,
        mob.circleOffsetX, mob.circleOffsetY, sty.size, editable, id
      );
...
export function createAnnotationToolbar(wholeSvg, chartContainer, scales, margin, chart_width, chart_height) {
  ...
  function renderAnnotationList(allAnnotations) {
    ...
    allAnnotations.forEach((annotation, index) => {
      ...
      // Annotation label
      item.append("span")
        .text(`${String.fromCharCode(65 + index)}: ${annotation.type} @ (${annotation.x | 0}, ${annotation.y | 0})`);
...
export async function loadAnnotationsFromJson(filePath, svgForChart, scales, isMobile = false) {
  try {
    ...
    let mobileAnnotationsCounter = 0;
    ...
    for (let anno of response) {
      if (anno.type !== 'direction-arrow') {
        anno.mobile = { 
          enabled: isMobile, 
          letter: indexToLetter(mobileAnnotationsCounter)
        };
...
export function createDelaunayOverlay({
  ...
  const instruction = createKeyboardInstruction();
  ...
  return {
    ...
    highlightAll: (pointIndex) => {
      if (pointIndex >= 0 && pointIndex < data.length) {
        ...
        updateHighlightAll(svgContainer, point, state, true, highlightFillColour);
      }
    },
  };
}
...
function createKeyboardInstruction() {
  const graphicNode = d3.select("#graphic").node();
  if (!graphicNode) return null;

  const parent = d3.select(graphicNode.parentNode);
  const existing = parent.select(".chart-keyboard-instruction");

  if (!existing.empty()) {
    existing.style("visibility", "hidden").text("");
    return existing;
  }

  return parent
    .insert("div", () => graphicNode.nextSibling)
    .attr("class", "chart-keyboard-instruction")
    .attr("role", "status")
...
function showKeyboardInstruction(instruction, message) {
  if (!instruction) return;
  instruction.style("visibility", "visible").text(message);
}

function hideKeyboardInstruction(instruction) {
  if (!instruction) return;
  instruction.style("visibility", "hidden").text("");
}
...
function setupMouseHandlers(
  ...
  function handlePointerSelection(event, resetHighlight) {
    ...
    if (state.currentHighlight !== nearestIndex) {
      ...
      multiHighlight
        ? updateHighlightAll(svgContainer, nearestPoint, state, false, highlightFillColour)
        : updateHighlight(
          svgContainer,
          nearestPoint,
          xScale,
  ...
  overlay.on("pointerdown", function (event) {
    state.lastInteraction = "pointer";
    state.isKeyboardMode = false;
    hideKeyboardInstruction(instruction);

    if (state.isDropdownMode) return;
    handlePointerSelection(event, true);
  });
...
function setupKeyboardHandlers(
  ...
  d3.select(svgElement).on("keydown", function (event) {
    ...
    showKeyboardInstruction(instruction, instructionMessage);
  ...
  d3.select(svgElement).on("focus", function () {
    if (state.lastInteraction === "pointer") {
  ...
  d3.select(svgElement).on("pointerdown", function () {
    state.lastInteraction = "pointer";
    state.isKeyboardMode = false;
    hideKeyboardInstruction(instruction);
  });
...
function updateHighlight(
  ...
  svgContainer
    ...
    .attr("fill", highlightFillColour || "none")
    .attr("stroke", "#222")
    .attr("stroke-width", 2)
    .style("pointer-events", "none");

    // Add a black outline on top of the orange and white shape
      svgContainer
        .append("path")
        .attr("class", "point-highlight")
        .attr("d", () => {
...
function updateHighlightAll(svgContainer, point, state, enableDropdownMode, highlightFillColour) {
  ...
  const matchingPoints = d3.selectAll(`.circle-${removeSpaces(areaId)}`);

  // Base highlight layer: orange fill with white stroke
  matchingPoints
    .clone(true)
    .attr("class", "point-highlight")
    .attr("fill", "#F39431")
  ...
  // Top highlight layer: black outline
  matchingPoints
    .clone(true)
    .attr("class", "point-highlight")
    .attr("fill", highlightFillColour || "none")
...
function createCleanupFunction(tooltip, overlay, svgContainer, state, instruction) {
 ...
 * @param {number} params.options.xOffset - Horizontal offset applied to all direct labels (default: 0)
 * @param {boolean} params.options.includeDataValue - Whether to include data values in labels (default: false)
 * @param {Function} params.options.valueFormatter - Formatter for data values (default: d3.format(",~f"))
 * @param {Object|null} params.options.colourMap - Map of categories to line colors (default: null)
 */
export function createDirectLabels({
  ...
  function getLabelDataPoint(data, category, strategy) {
    if (strategy === "firstLast") {
      ...
      for (let i = 0; i < data.length; i++) {
        if (data[i][category] !== null && data[i][category] !== undefined) {
          points.push({ datum: data[i], index: i, position: 'first' });
          break;
        }
      }
      ...
      for (let i = data.length - 1; i >= 0; i--) {
        if (data[i][category] !== null && data[i][category] !== undefined) {
          ...
          if (points.length === 0 || points[0].index !== i) {
            points.push({ datum: data[i], index: i, position: 'last' });
          }
          break;
        }
      }
      ...
      if (lastDatum[category] !== null && lastDatum[category] !== undefined) {
        return [{ datum: lastDatum, index: data.length - 1, position: 'last' }];
      }
      return [];
    } else {
      // 'lastValid'
      const result = findLastValidDataPoint(data, category);
      return result ? [{ ...result, position: "last" }] : []
    }
  }
  ...
  categories.forEach(function (category, index) {
    const labelPoints = getLabelDataPoint(data, category, labelStrategy);

    labelPoints.forEach(labelPoint => {
      const { datum, index: dataIndex, position } = labelPoint;
      ...
      if (includeDataValue) {
        ...
        if (position == "first") { labelText = [category, formattedValue]; }
        else { labelText = [formattedValue, category]; }
      }

      const lineColor = config.colourPalette[index % config.colourPalette.length];
      const textColor = adjustColorForContrast(lineColor, 4.5);
      const label = svg
        .append("text")
        ...
        .attr("y", yPos)
        .attr("dy", ".35em")
        .attr("text-anchor", anchor) // Adjust anchor based on position
        .attr(
          "fill",
          colourMap?.get(category) ?? textColor
        )
        .text(category)

      if (includeDataValue) {
        labelText.forEach((text, i) => {
          label.append("tspan")
            .attr("font-weight", position == "first" && i == 1 || position == "last" && i == 0 ? 700 : 400)
            .attr('dx', position == 'first' && i == 1 || position == 'last' && i == 1 ? 5 : 0)
            .text(text)
        })

        label.call(wrapStyled, position == "first" ? margin.left - 10 : margin.right - 10)
      } else {
        label.text(category)
          .call(wrap, position == "first" ? margin.left - 10 : margin.right - 10); // Use available margin space

      }

      // Get the actual height of the text element after wrapping
      label.node().getComputedTextLength();
      const bbox = includeDataValue ? getHeightWidthFromTextwithNestedTspan(label) : label.node().getBBox();
  ...
  // Helper function to resolve collisions and draw leader lines for a set of labels
  function processSideLabels(labels) {
    ...
    labels.forEach((label) => {
      ...
      if (useLeaderLines && Math.abs(label.y - label.originalY) > 1) {
        ...
        svg
          ...
          .attr(
            "stroke",
            colourMap?.get(label.category) ?? config.colourPalette[label.categoryIndex % config.colourPalette.length]
          )
          .attr("stroke-width", 1)
  ...
  // Filter and process each side
  processSideLabels(labelData.filter(label => label.position === "first"));
  processSideLabels(labelData.filter(label => label.position === "last"));


  return labelData; // Return label data in case caller needs it
}

export function getHeightWidthFromTextwithNestedTspan(textSel) {
  const textNode = textSel.node();
  const lineCount = +textSel.attr("data-line-count") || 1;

  const fontSize = parseFloat(
    window.getComputedStyle(textNode).fontSize
  );

  const lineHeightEm = 1.1; // MUST match wrap
  const height = lineCount * fontSize * lineHeightEm;
...
export function adjustColorForContrast(
  ...
  saturationBoost = 0.1
  // 0-1 Increase saturation to nake colors more vivid
) {
...
/**
 * Adds a data label with a white rectangle background for readability.
 * @param {Object} params
 * @param {d3.Selection} params.selection - The parent selection (e.g., svg or g).
 * @param {Array} params.data - The data array to bind.
 * @param {function} params.valueAccessor - Function to get label text from datum.
 * @param {function} params.xAccessor - Function to get x position from datum.
 * @param {function} params.yAccessor - Function to get y position from datum.
 * @param {function} params.dxAccessor - Function to get dx for text from datum.
 * @param {function} params.anchorAccessor - Function to get text-anchor from datum.
 * @param {function} params.fillAccessor - Function to get text fill color from datum.
 * @param {function} params.fontWeightAccessor - Function to get font weight from datum.
 */
export function addLabelWithBackground({
  selection,
  data,
  valueAccessor,
  xAccessor,
  ...
  // Join a group for each label, using labelType for unique class
  const labelGroups = selection
    .selectAll(`g.dataLabelGroup.${labelType}`)
    .data(data, d => d.name + labelType)
    .join("g")
    .attr("class", `dataLabelGroup ${labelType}`)
    .attr("transform", (d) => `translate(${xAccessor(d)},${yAccessor(d)})`);

  // Add text first (to measure bbox)
  labelGroups.each(function (d) {
    const g = d3.select(this);
    g.selectAll("text").remove();
    g.selectAll("rect").remove();
    g.append("text")
      .attr("class", "dataLabels tempMeasure")
      .attr("dy", 6)
      .attr("dx", dxAccessor(d))
      .attr("text-anchor", anchorAccessor(d))
      .style("font-weight", fontWeightAccessor(d))
      .style("opacity", 0)
      .text(valueAccessor(d));
  });

  // Now add rect and visible text
  labelGroups.each(function (d) {
    const g = d3.select(this);
    const text = g.select("text.tempMeasure");
    const node = text.node();
    let bbox = { width: 24, height: 16, x: 0, y: 0 };
    if (node) {
      bbox = node.getBBox();
    }
    if (background) {
...
function getScaleType(scale) {
  if (typeof scale.bandwidth === "function") return "band";
  if (typeof scale.ticks === "function") return "time";
  throw new Error("Unsupported scale type");
}

const customTemporalAxisBaseDomainKey = Symbol("customTemporalAxisBaseDomain");

function cloneTimeDomain(domain) {
  return domain.map(d => new Date(d));
}

function timeDomainsEqual(domainA, domainB) {
  return Array.isArray(domainA) &&
    Array.isArray(domainB) &&
    domainA.length === domainB.length &&
    domainA.every((d, i) => +d === +domainB[i]);
}

function getExpandedTimeDomain(domain, primaryLevel, forceFullLastPrimaryUnit = false) {
  const nextDomain = cloneTimeDomain(domain);

  if (!forceFullLastPrimaryUnit || !primaryLevel || nextDomain.length < 2) {
    return nextDomain;
  }

  const domainEnd = nextDomain[nextDomain.length - 1];
  const finalBoundary = primaryLevel.interval.floor(domainEnd);

  nextDomain[nextDomain.length - 1] = primaryLevel.interval.offset(finalBoundary, 1);
  return nextDomain;
}

function isMajorTick(d, secondaryLevel) {
  if (!secondaryLevel) return false;

  // For custom interval: floor to parent interval
  if (secondaryLevel.interval && typeof secondaryLevel.interval.floor === "function") {
    const parentFloor = secondaryLevel.interval.floor(new Date(d));
    return parentFloor.getTime() === new Date(d).getTime();
  }

  // Standard D3 interval: parent interval is standard D3
  return secondaryLevel.interval.floor(d).getTime() === new Date(d).getTime();
};


function buildTimeHierarchy({ yearStartMonth = 0 } = {}) {
  // --------------------------------
  // Custom fiscal/academic year interval
  // --------------------------------
  function customYearInterval(startMonth) {
    const floor = d =>
      d3.utcMonth.floor(
        d3.utcMonth.offset(
          d,
          -((d.getUTCMonth() - startMonth + 12) % 12)
        )
      );

    const offset = (d, step) =>
      d3.utcMonth.offset(floor(d), step * 12);

    const range = (start, stop) => {
      const dates = [];
      let current = floor(start);
    ...
    return { floor, offset, range };
  }

  // --------------------------------
  // Custom fiscal quarter interval
  // --------------------------------
  function customQuarterInterval(startMonth) {
    const floor = d => {
      const shifted =
        (d.getUTCMonth() - startMonth + 12) % 12;
      const q = Math.floor(shifted / 3) * 3;
      return d3.utcMonth.floor(
        d3.utcMonth.offset(d, -(shifted - q))
      );
    };

    const offset = (d, step) =>
      d3.utcMonth.offset(floor(d), step * 3);
  ...
  const yearInterval = customYearInterval(yearStartMonth);
  const quarterInterval = customQuarterInterval(yearStartMonth);

  return {
    day: {
      interval: d3.utcDay,
      format: d3.utcFormat("%d"),
      key: d => d.getUTCDate(),
      parent: "month"
    },
    ...
    quarter: {
      ...
      format: d => {
        const shifted =
          (d.getUTCMonth() - yearStartMonth + 12) % 12;
        return `Q${Math.floor(shifted / 3) + 1}`;
      },
      key: d => {
        const shifted =
          (d.getUTCMonth() - yearStartMonth + 12) % 12;
        return Math.floor(shifted / 3);
      },
      parent: "year"
    },
    ...
    year: {
      ...
      format: d => {
        const y = yearInterval.floor(d).getUTCFullYear();
        return yearStartMonth === 0
          ? `${y}`
          : `${y}/${(y + 1).toString().slice(-2)}`;
      },
      key: d => yearInterval.floor(d).getUTCFullYear(),
      parent: null
    }
  };
}
...
function buildTimeRanges(ticks, domainStart, domainEnd, finalRangeLevel = null) {
  const ranges = [];
  const rangeTicks = [...ticks];

  if (
    finalRangeLevel &&
    rangeTicks.length &&
    +finalRangeLevel.interval.floor(domainEnd) === +domainEnd &&
    +rangeTicks[rangeTicks.length - 1] === +domainEnd
  ) {
    rangeTicks.push(finalRangeLevel.interval.offset(domainEnd, 1));
  }

  if (!rangeTicks.length) return ranges;

  if (rangeTicks[0] > domainStart) {
    ranges.push({ start: domainStart, end: rangeTicks[0] });
  }
  ...
  if (rangeTicks[rangeTicks.length - 1] < domainEnd) {
    ranges.push({
      start: rangeTicks[rangeTicks.length - 1],
      end: domainEnd
    });
  }
  ...
  return ranges.map(r => ({
    ...r,
    mid: new Date((r.start.getTime() + r.end.getTime()) / 2)
  }));
}
...
function renderTimeTicks({
  ...
  minorStroke = ONScolours.grey20,
  majorStroke = ONScolours.grey40
}) {
  const sign = orient === "bottom" ? 1 : -1;

  // -----------------------------
  // 1. Generate ticks safely
  // -----------------------------
  let ticks;

  // If interval has 'range', assume custom interval
  if (primaryLevel.interval && typeof primaryLevel.interval.range === "function") {
    const domainStart = new Date(scale.domain()[0]);
    const domainEnd = new Date(scale.domain()[scale.domain().length - 1] || scale.domain()[1]);
    ticks = primaryLevel.interval.range(domainStart, domainEnd);
  } else {
    // Standard D3 interval: scale.ticks(interval) returns numbers or Dates
    ticks = scale.ticks(primaryLevel.interval).map(d => new Date(d));
  }
...
function buildParentRanges(ticks, parentLevel, domainStart, domainEnd) {
  const groups = d3.groups(ticks, d =>
    parentLevel.key(parentLevel.interval.floor(d))
  );

  return groups.map(([, group]) => {
    const floored = parentLevel.interval.floor(group[0]);

    const start = d3.max([floored, domainStart]);
    const end = d3.min([
      parentLevel.interval.offset(floored, 1),
      domainEnd
    ]);
...
function buildBandParentRanges(domain, parentLevel) {
  const groups = d3.groups(
    domain,
    d => parentLevel.interval.floor(d).getTime()
  );
  ...
  return groups.map(([, values]) => {
    ...
    const startDate = parentLevel.interval.floor(first);
    const endDate = parentLevel.interval.offset(startDate, 1);
...
function renderRangeLabels({
  ...
  const visible = ranges.filter(r =>
    Math.abs(scale(r.end) - scale(r.start)) >= minWidth
  );
  ...
  layer.selectAll("text")
    .data(visible)
    .join("text")
    .attr("x", d => scale(d.mid))
    .attr("y", 0)
    .attr("text-anchor", "middle")
    .attr("dy", "0.71em")
    .style("font-weight", bold ? 600 : null)
    .text(d => format(d.mid));
...
function renderBandLabels({
  ...
  // -----------------------------
  const temp = g.append("g").attr("class", "temp-labels").style("visibility", "hidden");
  ...
  const textWidths = temp.selectAll("text").nodes().map(node => node.getBBox().width);
  temp.remove();

  // Add width info
  labelData.forEach((d, i) => d.textWidth = textWidths[i]);

  // -----------------------------
  // 3. Filter by min width
  // -----------------------------
  const allFit = labelData.every(d => d.textWidth <= step);

  let visible;
  // Fallback: first/last if nothing fits
  if (!allFit) {
    const first = labelData[0];
    const last = labelData[labelData.length - 1];
    visible = [last];
    if (first !== last) visible.push(first);
  } else {
    visible = labelData
  }
...
function renderBandParentLabels({
  g,
  scale,
  domain,
  parentLevel,
  ...
  const bw = scale.bandwidth();
  const padding = scale.step() - bw;

  const groups = d3.groups(domain, d =>
    parentLevel.interval.floor(d).getTime()
  );

  const ranges = groups.map(([, values]) => {
    const first = values[0];
    const last = values[values.length - 1];

    const start = scale(first) - padding / 2;
    const end = scale(last) + bw + padding / 2;

    return {
      mid: start + (end - start) / 2,
      date: parentLevel.interval.floor(first),
      width: end - start
    };
  });
  ...
  const layer = g.append("g")
    .attr("class", "axis-parent-label-layer")
    .attr("transform", `translate(0, ${offset})`);
...
function resolveLevels({
  ...
  if (primaryUnitOverride) {
    primaryLevel = hierarchy[primaryUnitOverride];
    if (!primaryLevel) {
      throw new Error(`Invalid primary unit: ${primaryUnitOverride}`);
    }
  } else {
    // automatic defaults
    if (scaleType === "band") {
      const spanDays = (domain[domain.length - 1] - domain[0]) / (1000 * 60 * 60 * 24)

      primaryLevel = spanDays <= 31
        ? hierarchy.day
        : spanDays <= 365
          ? hierarchy.month
          : hierarchy.year;
      ...
      const spanMonths =
        (domain[1] - domain[0]) / (1000 * 60 * 60 * 24 * 30.44);
  ...
  const secondaryLevel =
    secondaryTimeUnit === false
      ? null
      : secondaryTimeUnit == null || secondaryTimeUnit == "" || secondaryTimeUnit == "auto" || secondaryTimeUnit == "parent"
        ? hierarchy[primaryLevel.parent]
        : secondaryTimeUnit != null && hierarchy[secondaryTimeUnit]
          ? hierarchy[secondaryTimeUnit]
          : null;

  return { primaryLevel, secondaryLevel };
}

function drawBandParentBoundaries({
  g,
  scale,
  domain,
  parentLevel,
  ...
  if (!parentLevel || domain.length === 0) return;

  const sign = orient === "bottom" ? 1 : -1;
  const bandwidth = scale.bandwidth();
  const step = scale.step();
  const paddingInnerPx = step - bandwidth;

  const domainStart = domain[0];
  const domainEnd = domain[domain.length - 1];

  const parentRanges = buildBandParentRanges(domain, parentLevel);
  const boundaries = new Set();

  parentRanges.forEach(r => {
    const fullyInside =
      r.startDate >= domainStart &&
      r.endDate <= d3.utcDay.offset(domainEnd, 1);

    const overlaps =
      r.endDate > domainStart &&
      r.startDate < d3.utcDay.offset(domainEnd, 1);

    if (!overlaps) return;

    // whole parent unit
    if (fullyInside) {
      boundaries.add(r.startDate.getTime());
      boundaries.add(r.endDate.getTime());
      return;
    }

    // domain enters parent
    if (domainStart > r.startDate && domainStart < r.endDate) {
      boundaries.add(r.endDate.getTime());
    }

    // domain exits parent
    if (domainEnd >= r.startDate && domainEnd < r.endDate) {
      boundaries.add(r.startDate.getTime());
    }
  });
  ...
  boundaries.forEach(t => {
    ...
    const idx = domain.findIndex(d => d >= date);
    if (idx === -1) return;

    const d = domain[idx];

    // left boundary
    if (parentLevel.interval.floor(d).getTime() === t) {
      xPositions.push(scale(d) - paddingInnerPx / 2);
    }

    // right boundary
    const prev = domain[idx - 1];
    if (
      prev &&
      parentLevel.interval.offset(
        parentLevel.interval.floor(prev),
        1
      ).getTime() === t
    ) {
      xPositions.push(
        scale(prev) + bandwidth + paddingInnerPx / 2
      );
    }
  });
  ...
  g.append("g")
    .attr("class", "axis-parent-boundaries")
    .selectAll("line")
    .data(xPositions)
    .join("line")
    .attr("x1", d => d)
...
export function customTemporalAxis(scaleInput) {
  let scale = scaleInput || null;
  let orient = "bottom";
  let tickSize = 10;
  let tickPadding = 6;
  let minLabelWidth = 24;

  let primaryUnitOverride = null;
  let primaryFormatOverride = null;
  let secondaryTimeUnit = "parent"; // "parent" | null
  let secondaryFormatOverride = null;

  let yearStartMonth = 0;
  let forceFullLastPrimaryUnit = false;

  function axis(selection) {
    selection.each(function () {
      if (!scale) throw new Error("customTemporalAxis requires a scale");

      const scaleType = getScaleType(scale);
      const hierarchy = buildTimeHierarchy({ yearStartMonth });
      const levelDomain = scaleType === "time" && scale[customTemporalAxisBaseDomainKey]
        ? cloneTimeDomain(scale[customTemporalAxisBaseDomainKey])
        : scale.domain();

      const { primaryLevel, secondaryLevel } = resolveLevels({ hierarchy, domain: levelDomain, scaleType, primaryUnitOverride, secondaryTimeUnit })

      if (scaleType === "time") {
        expandCustomTemporalAxisDomain(scale, {
          timeUnit: primaryUnitOverride,
          yearStartMonth,
          forceFullLastPrimaryUnit
        });
      }
      ...
      if (scaleType === "time") {
        renderTimeAxis(g, scale, primaryLevel, secondaryLevel, primaryFormatOverride, secondaryFormatOverride);
      } else {
        renderBandAxis(g, scale, primaryLevel, secondaryLevel, primaryFormatOverride, secondaryFormatOverride);
      }
    });
  }
  ...
  // TIME SCALE RENDERER
  // -----------------------------
  function renderTimeAxis(g, scale, primaryLevel, secondaryLevel, primaryFormatOverride, secondaryFormatOverride) {
    const [start, end] = scale.domain();

    const ticks = primaryLevel.interval.range(
      primaryLevel.interval.floor(start),
      end
    );
    ...
    const timeRanges = buildTimeRanges(
      ...
      forceFullLastPrimaryUnit ? primaryLevel : null
    );
    ...
    renderTimeTicks({
      ...
      primaryLevel,
      secondaryLevel,
      minorTickSize: tickSize,
      majorTickSize: tickSize * 2.5
    });

    let yOffset = tickPadding;

    renderRangeLabels({
      g,
      scale,
      ranges: timeRanges,
      format: primaryFormatOverride || primaryLevel.format,
    ...
    if (secondaryLevel) {
      renderRangeLabels({
        g,
        scale,
        ranges: buildParentRanges(ticks, secondaryLevel, start, end),
        format: secondaryFormatOverride || secondaryLevel.format,
        minWidth: minLabelWidth * 1.2,
        offset: yOffset,
        bold: true
      });
    }
  ...
  // BAND SCALE RENDERER
  // -----------------------------
  function renderBandAxis(g, scale, primaryLevel, secondaryLevel, primaryFormatOverride, secondaryFormatOverride) {
    const domain = scale.domain();

    const bandCenter = d => scale(d) + scale.bandwidth() / 2;
    const sign = orient === "bottom" ? 1 : -1;
    ...
    g.selectAll("line")
      ...
      .attr("x2", bandCenter)
      .attr("y1", 0)
      .attr("y2", tickSize * sign)
      .attr("stroke", ONScolours.grey20);

    let yOffset = tickSize + tickPadding;
    ...
    if (secondaryLevel) {
      drawBandParentBoundaries({
        g,
        scale,
        domain,
        parentLevel: secondaryLevel,
        orient,
        height: yOffset + 14
      });


      renderBandParentLabels({
        g,
        scale,
        domain,
        parentLevel: secondaryLevel,
  ...
  // API
  // -----------------------------
  axis.scale = _ => arguments.length ? (scale = _, axis) : scale;
  axis.orient = _ => arguments.length ? (orient = _, axis) : orient;
  axis.tickSize = _ => arguments.length ? (tickSize = +_, axis) : tickSize;
  axis.tickPadding = _ => arguments.length ? (tickPadding = +_, axis) : tickPadding;

  axis.timeUnit = _ =>
    arguments.length ? (primaryUnitOverride = _, axis) : primaryUnitOverride;

  axis.secondaryTimeUnit = _ =>
    arguments.length ? (secondaryTimeUnit = _, axis) : secondaryTimeUnit;

  axis.tickFormat = _ =>
    arguments.length ? (primaryFormatOverride = _, axis) : primaryFormatOverride;

  axis.secondaryTickFormat = _ =>
    arguments.length ? (secondaryFormatOverride = _, axis) : secondaryFormatOverride;

  axis.yearStartMonth = _ =>
    arguments.length ? (yearStartMonth = +_, axis) : yearStartMonth;

  axis.forceFullLastPrimaryUnit = _ =>
    arguments.length ? (forceFullLastPrimaryUnit = !!_, axis) : forceFullLastPrimaryUnit;
...
export function expandCustomTemporalAxisDomain(scale, {
  timeUnit = null,
  yearStartMonth = 0,
  forceFullLastPrimaryUnit = false
} = {}) {
  if (!scale || getScaleType(scale) !== "time") return scale;

  const currentDomain = cloneTimeDomain(scale.domain());
  const storedBaseDomain = scale[customTemporalAxisBaseDomainKey]
    ? cloneTimeDomain(scale[customTemporalAxisBaseDomainKey])
    : null;
  const baseDomain = storedBaseDomain || currentDomain;
  const hierarchy = buildTimeHierarchy({ yearStartMonth });
  const { primaryLevel } = resolveLevels({
    hierarchy,
    domain: baseDomain,
    scaleType: "time",
    primaryUnitOverride: timeUnit,
    secondaryTimeUnit: "parent"
  });

  if (!storedBaseDomain) {
    scale[customTemporalAxisBaseDomainKey] = cloneTimeDomain(currentDomain);
  } else {
    const expectedExpandedDomain = getExpandedTimeDomain(
      storedBaseDomain,
      primaryLevel,
      true
    );
    ...
    if (
      !timeDomainsEqual(currentDomain, storedBaseDomain) &&
      !timeDomainsEqual(currentDomain, expectedExpandedDomain)
    ) {
      scale[customTemporalAxisBaseDomainKey] = cloneTimeDomain(currentDomain);
    }
  }

  const resolvedBaseDomain = cloneTimeDomain(scale[customTemporalAxisBaseDomainKey]);
  const nextDomain = getExpandedTimeDomain(
    resolvedBaseDomain,
    primaryLevel,
    forceFullLastPrimaryUnit
  );
...
export function prefixYearFormatter(d, yearStartMonth, prefix) {
  const y = d.getUTCFullYear();
  const m = d.getUTCMonth();

  const fyStartYear = m >= yearStartMonth ? y : y - 1;
  const fyEndYear = fyStartYear + 1;

  return `${prefix}${fyStartYear}/${String(fyEndYear).slice(-2)}`;
}

export function quarterYearFormatter(d, yearStartMonth = 0) {
  const y = d.getUTCFullYear();
  const m = d.getUTCMonth();
  const q = Math.floor(((m - yearStartMonth + 12) % 12) / 3) + 1
  return `Q${q} ${y}`
}

function sampleSvgPathByPixel(pathElement, totalWidth, sampleStep = 0.5) {
  const totalLength = pathElement.getTotalLength();
  const yRanges = new Map();

  const addPointToRange = ({ x, y }) => {
    const xPixel = Math.round(x);
    if (xPixel < 0 || xPixel > Math.ceil(totalWidth)) return;

    const existing = yRanges.get(xPixel);
    if (!existing) {
      yRanges.set(xPixel, { minY: y, maxY: y });
      return;
    }
  ...
  for (let length = 0; length <= totalLength; length += sampleStep) {
    addPointToRange(pathElement.getPointAtLength(length));
  }

  addPointToRange(pathElement.getPointAtLength(totalLength));

  return yRanges;
}

function buildCiBandPixelSpans({
  svgNode,
  data,
  category,
  xScale,
  ...
  const defined = (d) =>
    d[`${category}_lowerCI`] !== null &&
    d[`${category}_upperCI`] !== null;

  const curve = d3[curveType] || d3.curveLinear;
  const makeBoundaryPath = (key) =>
    d3
      .line()
  ...
  const upperPath = makeBoundaryPath(`${category}_upperCI`);
  const lowerPath = makeBoundaryPath(`${category}_lowerCI`);

  if (!upperPath || !lowerPath) return new Map();

  const upperPathElement = document.createElementNS("http://www.w3.org/2000/svg", "path");
  const lowerPathElement = document.createElementNS("http://www.w3.org/2000/svg", "path");

  upperPathElement.setAttribute("d", upperPath);
  lowerPathElement.setAttribute("d", lowerPath);
  svgNode.appendChild(upperPathElement);
  svgNode.appendChild(lowerPathElement);

  try {
    const width =
      typeof xScale.range === "function" ? Math.max(...xScale.range()) : 0;
    const upperRanges = sampleSvgPathByPixel(upperPathElement, width, sampleStep);
    const lowerRanges = sampleSvgPathByPixel(lowerPathElement, width, sampleStep);
    const xPixels = new Set([...upperRanges.keys(), ...lowerRanges.keys()]);
    const bandSpans = new Map();

    xPixels.forEach((xPixel) => {
      const upperRange = upperRanges.get(xPixel);
      const lowerRange = lowerRanges.get(xPixel);
      if (!upperRange || !lowerRange) return;

      bandSpans.set(xPixel, {
        topY: Math.min(
          upperRange.minY,
          upperRange.maxY,
          lowerRange.minY,
    ...
    upperPathElement.remove();
    lowerPathElement.remove();
  }
}

export function getCiAreaOverlapFlags({
  svgNode,
  data,
  categories,
  xScale,
  ...
  xAccessor = (d) => d.date,
  sampleStep = 0.5,
  overlapThresholdPx = 0.5,
}) {
  const overlapFlags = Object.fromEntries(categories.map((category) => [category, false]));

  if (categories.length < 2) {
    return overlapFlags;
  }

  const bandSpansByCategory = new Map(
    categories.map((category) => [
      category,
      buildCiBandPixelSpans({
        svgNode,
        data,
        category,
        xScale,
  ...
  for (let i = 0; i < categories.length; i += 1) {
    for (let j = i + 1; j < categories.length; j += 1) {
      ...
      const spansA = bandSpansByCategory.get(categoryA);
      const spansB = bandSpansByCategory.get(categoryB);

      if (!spansA?.size || !spansB?.size) continue;

      const [smallerSpans, largerSpans] =
        spansA.size <= spansB.size ? [spansA, spansB] : [spansB, spansA];

      let hasOverlap = false;

      for (const [xPixel, spanA] of smallerSpans.entries()) {
        const spanB = largerSpans.get(xPixel);
        if (!spanB) continue;

        const overlapHeight =
          Math.min(spanA.bottomY, spanB.bottomY) -
          Math.max(spanA.topY, spanB.topY);

        if (overlapHeight > overlapThresholdPx) {
          overlapFlags[categoryA] = true;
          overlapFlags[categoryB] = true;
          hasOverlap = true;
          break;
        }
      }
