...
export function createDelaunayOverlay({
  ...
  setupMouseHandlers(overlay, delaunay, data, xScale, yScale, tooltip, config, state, shape, getSymbolSize, sizeScale, radius, svgContainer, multiHighlight);
...
function setupMouseHandlers(overlay, delaunay, data, xScale, yScale, tooltip, config, state, shape, getSymbolSize, sizeScale, radius, svgContainer, multiHighlight) {
  overlay.on('mousemove', function (event) {
    ...
    if (state.currentHighlight !== nearestIndex) {
      ...
      multiHighlight ? updateHighlightAll(svgContainer, nearestPoint, state) : updateHighlight(svgContainer, nearestPoint, xScale, yScale, shape, getSymbolSize, sizeScale);
...
export function customTimeAxis(scaleInput) {
  ...
  function axis(selection) {
    selection.each(function () {
      if (!scale) throw new Error("customTimeAxis requires a scale (e.g. customTimeAxis(x))");

      const [start, end] = scale.domain();
      const domainSpanMs = end - start;
      const domainSpanMonths = domainSpanMs / (1000 * 60 * 60 * 24 * 30.44);

      // --- Determine base interval dynamically ---
      let interval, defaultFormat, periodMs;
      if (domainSpanMonths <= 12) {
        interval = d3.utcMonth;
        defaultFormat = d3.utcFormat("%b %Y");
        periodMs = 1000 * 60 * 60 * 24 * 30.44;
      } else if (domainSpanMonths <= 36) {
        interval = d3.utcMonth.every(3);
        defaultFormat = d => `Q${Math.floor(d.getUTCMonth() / 3) + 1} ${d.getUTCFullYear()}`;
        periodMs = 1000 * 60 * 60 * 24 * 91.31;
      } else {
        interval = d3.utcYear;
        defaultFormat = d3.utcFormat("%Y");
        periodMs = 1000 * 60 * 60 * 24 * 365.25;
      }

      // --- Determine ticks ---
      let ticks = userTickCount
        ? scale.ticks(userTickCount) // user override
        : scale.ticks(interval);

      // --- Compute midpoints for label positions ---
      const labels = [];
      ...
      if (ticks.length) {
        if (ticks[0].getTime() - start.getTime() > periodMs / 2)
          labels.unshift(new Date(ticks[0].getTime() - periodMs / 2));
        const lastTick = ticks[ticks.length - 1];
        if (end.getTime() - lastTick.getTime() > periodMs / 2)
          labels.push(new Date(lastTick.getTime() + periodMs / 2));
      }

      const g = d3.select(this);

      // 1️⃣ Tick marks (no text)
      ...
      // 2️⃣ Label layer
      const labelAxis = (orient === "top" ? d3.axisTop : d3.axisBottom)(scale)
        .tickValues(labels)
        .tickSize(0)
        .tickPadding(tickPadding)
        .tickFormat(
      ...
      gLabel.selectAll("path").remove();
      gLabel.selectAll("line").remove();
    });
  }

  // --- Chainable D3-style configuration API ---
  ...
  axis.tickFormat = function (_) {
    return arguments.length ? ((userTickFormat = _), axis) : userTickFormat;
  };

  axis.ticks = function (_) {
    return arguments.length ? ((userTickCount = +_), axis) : userTickCount;
  };
