...
function drawBandParentBoundaries({
  ...
  if (!parentLevel || !domain.length) return;
  ...
  // -------------------------------------------------
  // 1. Build TRUE parent ranges from domain extent
  // -------------------------------------------------
  const parentRanges = parentLevel.interval.range(
    parentLevel.interval.floor(domainStart),
    parentLevel.interval.offset(domainEnd, 1)
  ).map(start => {
    const end = parentLevel.interval.offset(start, 1);
    return { start, end };
  });

  // -------------------------------------------------
  // 2. Keep only parent ranges that are fully visible
  // -------------------------------------------------
  const visibleWholeRanges = parentRanges.filter(r =>
    r.start >= domainStart && r.end <= d3.utcDay.offset(domainEnd, 1)
  );

  if (!visibleWholeRanges.length) return;

  // -------------------------------------------------
  // 3. Convert parent ranges to band-space extents
  // -------------------------------------------------
  const extents = visibleWholeRanges
    .map(r => {
      // find primary domain values inside this parent range
      const values = domain.filter(
        d => d >= r.start && d < r.end
      );
      ...
      const startX = scale(first) - paddingInnerPx / 2;
      const endX = scale(last) + bandwidth + paddingInnerPx / 2;
  ...
  extents.forEach(r => {
    boundarySet.add(r.startX);
    boundarySet.add(r.endX);
  });
