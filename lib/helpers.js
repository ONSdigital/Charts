...
export function addDataLabels({
  ...
  const barStart = d =>
    Math.min(xScaleFunction(0), xScaleFunction(d.value));

  const barEnd = d =>
    Math.max(xScaleFunction(0), xScaleFunction(d.value));
  ...
  svgContainer
    ...
    .attr('x', d => isSmall(d) ? x0 : xScaleFunction(d.value))
    .attr('x', d =>
      isSmall(d)
        ? (d.value > 0 ? barEnd(d) + padding : barStart(d) - padding)
        : (d.value > 0 ? barEnd(d) - padding : barStart(d) + padding)
    )
    .attr('y', d =>
      y2function
...
export function createDirectLabels({
  ...
  categories.forEach(function (category, index) {
    ...
    if (dataIndex === data.length - 1) {
      ...
      const chartWidth = xScale.range()[1]+margin.right;
