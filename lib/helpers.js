 ...
 * @param {string} params.options.labelStrategy - Strategy for positioning labels: 'last', 'lastValid', or 'lastValidRight' (default: 'lastValid')
...
export function createDirectLabels({
   ...
   * @param {string} category - Category name
     * @param {string} strategy - 'last', 'lastValid', or 'lastValidRight'
  ...
  function getLabelDataPoint(data, category, strategy) {
    if (strategy === "last") {
      ...
      // 'lastValid' (and variants that still use last-valid selection)
  ...
  categories.forEach(function (category, index) {
    ...
    if (labelStrategy === "lastValidRight" && endedEarly) {
      // If a line ends early, place its label in the right margin and connect via the chart edge.
  ...
  function forceLeaderLineForLabel(label) {
    return labelStrategy === "lastValidRight" && label.endedEarly === true;
  }
  ...
  function needsLeaderLineForLabel(label) {
    ...
    // …or when 'lastValidRight' intentionally routes to the chart edge.
  ...
  function shouldUseGapWithLeaderLines(label) {
    ...
    // For 'lastValidRight' early-ending series, do NOT apply the larger gap
  ...
  labelData.forEach((label) => {
    ...
    if (useLeaderLines && needsLeaderLineForLabel(label)) {
      ...
      // For early-ending series in 'lastValidRight', route the leader via the chart's right edge.
      // Otherwise, use a small elbow offset from the data point.
      const preferredVerticalX =
        labelStrategy === "lastValidRight" && label.endedEarly === true
          ? xScale.range()[1]
          : null;
      ...
      // Special case: lastValidRight early-ending series that are not displaced/clustered.
