...
export function addDataLabels({
  ...
  svgContainer
    ...
    .attr("fill", (d) => {
      const isOutside = Math.abs(xScaleFunction(d.value) - xScaleFunction(0)) <
        chartWidth / labelPositionFactor;
      if (isOutside) {
        return "#414042";
      } else {
 ...
 * @param {string} params.options.leaderLineColourMode - 'series' to match series colour or 'mono' for a single colour (default: 'series')
 * @param {string} params.options.leaderLineMonoColour - Colour used when leaderLineColourMode is 'mono' (default: '#707070')
 * @param {number} params.options.leaderLineElbowOffset - Horizontal offset from the data point to the vertical leader segment (default: 10)
 * @param {number} params.options.leaderLineEndGap - Small gap before the label edge where the leader terminates (default: 2)
 * @param {number} params.options.labelGap - Horizontal gap (px) between the series endpoint and the direct label anchor (default: 10)
 * @param {number} params.options.labelGapWithLeaderLines - Horizontal gap (px) for labels that require leader lines (default: labelGap)
 * @param {string} params.options.labelLocation - Where the direct label should live.
 *   Allowed values:
 *   - 'margin': label sits in the right margin, aligned to final x.
 *   - 'lastPoint': label sits just right of the series endpoint (last valid point).
 *   - 'marginLeader': label sits in right margin; early-ending series get a leader routed via chart edge.
 *   (default: 'lastPoint')
...
export function createDirectLabels({
  ...
  const {
    ...
    leaderLineEndGap = 2,
    labelGap = 10,
    labelGapWithLeaderLines = labelGap,
    labelLocation,
    minLabelOffset = 5,
  } = options;

  function normaliseLabelLocation(location) {
    if (!location) return "lastPoint";
    if (location === "margin") return "margin";
    if (location === "marginLeader") return "marginLeader";
    if (location === "lastPoint") return "lastPoint";
    return "lastPoint";
  }

  const resolvedLocation = normaliseLabelLocation(labelLocation);

  // Remove any existing direct labels and leader lines before adding new ones
  svg.selectAll("text.directLineLabel").remove();
  svg.selectAll(".label-leader-line").remove();

  function clamp(value, min, max) {
    return Math.max(min, Math.min(max, value));
  }

  function getLeaderStroke(label) {
    if (leaderLineColourMode === "mono") return leaderLineMonoColour;
    return config.colourPalette[label.categoryIndex % config.colourPalette.length];
  }

  function getLabelEdgeXFromBBox(labelSelection, bbox) {
    const anchor = labelSelection.attr("text-anchor") || "start";
    return anchor === "end" ? bbox.x + bbox.width : bbox.x;
  }

  function updateLabelY(labelSelection, newY) {
    labelSelection.attr("y", newY);
    // If the label is multi-line (tspans), keep them aligned when y changes
    labelSelection.selectAll("tspan").attr("y", newY);
  }

  function getRightBoundX() {
    return xScale.range()[1] + (margin?.right ?? 0) - minLabelOffset;
  }

  function getLeftBoundX() {
    return minLabelOffset;
  }

  function getMinWrapWidth() {
    return 30;
  }

  function clampLabelXForWrap(anchor, x) {
    const leftBound = getLeftBoundX();
    const rightBound = getRightBoundX();
    const minWrapWidth = getMinWrapWidth();

    if (anchor === "end") {
      // 'end' anchor means the text extends left from x
      return clamp(x, leftBound + minWrapWidth, rightBound);
    }
    // 'start' anchor means the text extends right from x
    return clamp(x, leftBound, rightBound - minWrapWidth);
  }

  function getWrapWidth(anchor, x) {
    const leftBound = getLeftBoundX();
    const rightBound = getRightBoundX();
    const minWrapWidth = getMinWrapWidth();

    const width = anchor === "end" ? x - leftBound : rightBound - x;
    return Math.max(minWrapWidth, width);
  }

  function applyWrapAndMeasure(labelSelection, labelText, x, y) {
    const anchor = labelSelection.attr("text-anchor") || "start";
    const clampedX = clampLabelXForWrap(anchor, x);
    const wrapWidth = getWrapWidth(anchor, clampedX);

    labelSelection.attr("x", clampedX).attr("y", y);
    labelSelection.selectAll("tspan").remove();
    labelSelection.text(labelText);
    labelSelection.call(wrap2, wrapWidth, 0, 1.1, 1, true, "middle");
   ...
   * @param {string} location - 'margin', 'lastPoint', or 'marginLeader'
   * @returns {Object|null} - Object with {datum, index} or null
   */
  function getLabelDataPoint(data, category, location) {
    if (location === "margin") {
      // Prefer the final x point if available, otherwise fall back to last valid so a label
      // always renders (even if the series is missing on the last row).
    ...
    if (location === "lastPoint" || location === "marginLeader") {
      return findLastValidDataPoint(data, category);
    }

    // Unknown strategy - fall back safely
    return findLastValidDataPoint(data, category);
  }
  ...
  categories.forEach(function (category, index) {
    const labelPoint = getLabelDataPoint(data, category, resolvedLocation);
    ...
    const endedEarly = dataIndex !== data.length - 1;

    // Calculate label x position - offset from data point or chart edge
    let labelX;
    let placementMode; // 'right', 'left', or 'rightMargin'
    if (resolvedLocation === "margin" || resolvedLocation === "marginLeader") {
      // Always place the label in the right margin.
      labelX = chartWidth + labelGap;
      placementMode = "rightMargin";
    } else if (resolvedLocation === "lastPoint") {
      // Always place the label to the right of the series endpoint.
      labelX = xPos + labelGap;
      placementMode = "right";
    ...
    const label = svg
      ...
      .attr("y", yPos)
      .attr("dominant-baseline", "middle")
      .attr("text-anchor", placementMode === "left" ? "end" : "start")
      .attr("fill", textColor)
      .text(category);

    const wrapped = applyWrapAndMeasure(label, category, labelX, yPos);

    // Get the actual height of the text element after wrapping
    const bbox = wrapped.bbox;
  ...
  function forceLeaderLineForLabel(label) {
    return resolvedLocation === "marginLeader" && label.endedEarly === true;
  }

  function needsLeaderLineForLabel(label) {
    // Any label in a collision cluster should show a leader line for consistency
    // (e.g. when three labels are packed, the middle one also gets a leader line).
    if (label.inCluster) return true;

    // Otherwise, only when vertically displaced…
    if (Math.abs(label.y - label.originalY) > 1) return true;

    // …or when 'marginLeader' intentionally routes to the chart edge.
    return forceLeaderLineForLabel(label);
  }

  function shouldUseGapWithLeaderLines(label) {
    // Larger gap is used for labels that are actually displaced / clustered.
    // For 'marginLeader' early-ending series, do NOT apply the larger gap
    // unless the label is also being moved to accommodate other labels.
    return label.inCluster || Math.abs(label.y - label.originalY) > 1;
  }

  function desiredXForLabel(label) {
    const gap = shouldUseGapWithLeaderLines(label) ? labelGapWithLeaderLines : labelGap;

    if (label.placementMode === "rightMargin") {
      return xScale.range()[1] + gap;
    }
    if (label.placementMode === "left") {
      return label.dataX - gap;
    }
    return label.dataX + gap;
  }

  function layoutLabelYPositions() {
    if (labelData.length <= 1) return;

    // Reset cluster flags
    for (const label of labelData) {
      label.inCluster = false;
    }

    // Sort labels by original y for deterministic grouping
    const sorted = [...labelData].sort((a, b) => a.originalY - b.originalY);

    // Build overlap clusters (based on original positions)
    const clusters = [];
    let currentCluster = [];
    let clusterBottom = -Infinity;

    for (const label of sorted) {
      const top = label.originalY - label.height / 2;
      const bottom = label.originalY + label.height / 2;
      ...
      const needsCluster = top < clusterBottom + minSpacing;
      if (needsCluster) {
        currentCluster.push(label);
    ...
    // Compute adjusted y positions per cluster, aiming for symmetric offsets
    for (const cluster of clusters) {
      if (cluster.length <= 1) continue;

      // Mark all labels in this cluster
      for (const label of cluster) {
        label.inCluster = true;
      }

      const n = cluster.length;
      const newYs = new Array(n);

      if (n % 2 === 1) {
        // Odd: keep the middle label aligned to its point
        const mid = Math.floor(n / 2);
        newYs[mid] = cluster[mid].originalY;

        // Place above the middle
        for (let i = mid - 1; i >= 0; i--) {
          const below = cluster[i + 1];
          const gap = cluster[i].height / 2 + below.height / 2 + minSpacing;
          newYs[i] = newYs[i + 1] - gap;
        }

        // Place below the middle
        for (let i = mid + 1; i < n; i++) {
          const above = cluster[i - 1];
          const gap = cluster[i].height / 2 + above.height / 2 + minSpacing;
          newYs[i] = newYs[i - 1] + gap;
        }
      } else {
        // Even: center between the two middle labels
        const left = n / 2 - 1;
        const right = n / 2;
        const anchor = (cluster[left].originalY + cluster[right].originalY) / 2;
        const midGap =
          cluster[left].height / 2 + cluster[right].height / 2 + minSpacing;

        newYs[left] = anchor - midGap / 2;
        newYs[right] = anchor + midGap / 2;
      ...
      // Shift whole cluster into bounds, preserving spacing
      let minTop = Infinity;
      let maxBottom = -Infinity;
      for (let i = 0; i < n; i++) {
        minTop = Math.min(minTop, newYs[i] - cluster[i].height / 2);
        maxBottom = Math.max(maxBottom, newYs[i] + cluster[i].height / 2);
      }

      let shift = 0;
      if (minTop < 0) shift = -minTop;
      if (maxBottom + shift > chartHeight)
        shift -= maxBottom + shift - chartHeight;

      for (let i = 0; i < n; i++) {
        cluster[i].y = clamp(
          newYs[i] + shift,
          cluster[i].height / 2,
          chartHeight - cluster[i].height / 2
        );
      }
    }

    // Global collision pass:
    // If a cluster is pushed into bounds (e.g. near the top), it can end up overlapping labels
    // that were not originally in the same cluster. Resolve overlaps against the chart bounds
    // and between all labels using a couple of forward/backward sweeps.
    // Preserve the natural vertical ordering based on original positions so labels don't
    // unexpectedly swap order after cluster/bounds adjustments.
    const all = [...labelData].sort((a, b) => a.originalY - b.originalY);

    const minYFor = (label) => label.height / 2;
    const maxYFor = (label) => chartHeight - label.height / 2;

    const clampAllIntoBounds = () => {
      for (const label of all) {
        label.y = clamp(label.y, minYFor(label), maxYFor(label));
      }
    };

    const forwardSeparate = () => {
      for (let i = 1; i < all.length; i++) {
        const prev = all[i - 1];
        const cur = all[i];
        const minY =
          prev.y + prev.height / 2 + cur.height / 2 + minSpacing;
        if (cur.y < minY) cur.y = minY;
      }
    };

    const backwardSeparate = () => {
      for (let i = all.length - 2; i >= 0; i--) {
        const next = all[i + 1];
        const cur = all[i];
        const maxY =
          next.y - next.height / 2 - cur.height / 2 - minSpacing;
        if (cur.y > maxY) cur.y = maxY;
      }
    };

    // A few sweeps is enough for our small label counts and avoids over-complication.
    for (let iter = 0; iter < 3; iter++) {
      clampAllIntoBounds();
      forwardSeparate();
      // Ensure we don't push the stack past the bottom.
      all[all.length - 1].y = Math.min(all[all.length - 1].y, maxYFor(all[all.length - 1]));
      backwardSeparate();
      // Ensure we don't push the stack past the top.
      all[0].y = Math.max(all[0].y, minYFor(all[0]));
    }

    clampAllIntoBounds();
  }

  // Layout pass 1: compute y positions based on initial wrapping
  layoutLabelYPositions();

  // If leader-line labels use a bigger gap, apply it now and rewrap with updated widths
  // (then run layout again in case label heights changed due to wrapping).
  let anyRewrapped = false;
  for (const label of labelData) {
    const newX = desiredXForLabel(label);
    if (Math.abs(newX - label.x) > 0.5) {
      const wrapped = applyWrapAndMeasure(label.node, label.labelText, newX, label.originalY);
      label.x = wrapped.x;
      label.width = wrapped.bbox.width;
      label.height = wrapped.bbox.height;
      anyRewrapped = true;
    }
  }
  ...
  // Apply the final positions and draw orthogonal leader lines if needed
  labelData.forEach((label) => {
    updateLabelY(label.node, label.y);

    if (useLeaderLines && needsLeaderLineForLabel(label)) {
      const strokeDashArray = leaderLineStyle === "dashed" ? "2,2" : "none";
      const bbox = label.node.node().getBBox();
      const labelEdgeX = getLabelEdgeXFromBBox(label.node, bbox);
      const labelCenterY = bbox.y + bbox.height / 2;

      const dir = labelEdgeX >= label.dataX ? 1 : -1;

      // For early-ending series in 'marginLeader', route the leader via the chart's right edge.
      // Otherwise, use a small elbow offset from the data point.
      const preferredVerticalX =
        resolvedLocation === "marginLeader" && label.endedEarly === true
          ? xScale.range()[1]
          : null;

      const endX = labelEdgeX - dir * leaderLineEndGap;

      // Choose where the vertical segment sits.
      // For most cases, place it around the midpoint between the point and label edge so the two
      // horizontal segments are visually more symmetrical. Enforce a minimum offset from the point
      // so we don't get a tiny first segment.
      let elbowX;
      if (preferredVerticalX != null) {
        elbowX = preferredVerticalX;
      } else {
        const midpointX = (label.dataX + endX) / 2;
        const minFromPoint = label.dataX + dir * leaderLineElbowOffset;

        if (dir === 1) {
          elbowX = Math.max(minFromPoint, Math.min(midpointX, endX));
        } else {
          elbowX = Math.min(minFromPoint, Math.max(midpointX, endX));
        }
      }

      // Special case: marginLeader early-ending series that are not displaced/clustered.
      // Draw a single horizontal leader (no vertical segment) to avoid a tiny visual "kink".
      const isUndisplaced = Math.abs(label.y - label.originalY) <= 1;
      const simpleHorizontalLeader =
        resolvedLocation === "marginLeader" &&
        label.endedEarly === true &&
        label.inCluster !== true &&
        isUndisplaced;

      if (simpleHorizontalLeader) {
        svg
          .append("path")
          .attr("class", "label-leader-line")
          .attr("d", `M${label.dataX},${label.dataY} H${endX}`)
          .attr("fill", "none")
          .attr("stroke", getLeaderStroke(label))
          .attr("stroke-width", 1)
          .attr("stroke-dasharray", strokeDashArray);
        return;
      }

      // Leader: horizontal -> vertical -> horizontal
      svg
        .append("path")
        .attr("class", "label-leader-line")
        .attr("d", `M${label.dataX},${label.dataY} H${elbowX} V${labelCenterY} H${endX}`)
        .attr("fill", "none")
        .attr("stroke", getLeaderStroke(label))
        .attr("stroke-width", 1)
        .attr("stroke-dasharray", strokeDashArray);
...
function buildTimeRanges(ticks, domainStart, domainEnd) {
  const ranges = [];

  if (!ticks.length) return ranges;

  if (ticks[0] > domainStart) {
    ranges.push({ start: domainStart, end: ticks[0] });
  }
...
export function customTemporalAxis(scaleInput) {
  ...
  function axis(selection) {
    selection.each(function () {
      ...
      const { primaryLevel, secondaryLevel } = resolveLevels({ hierarchy, domain: scale.domain(), scaleType, primaryUnitOverride, secondaryTimeUnit })
  ...
  function renderTimeAxis(g, scale, primaryLevel, secondaryLevel, primaryFormatOverride, secondaryFormatOverride) {
    ...
    renderRangeLabels({
      g,
      scale,
      ranges: buildTimeRanges(ticks, start, end),
