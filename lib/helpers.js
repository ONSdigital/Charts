 ...
 * @param {string} params.config.text_colour_palette - Array of text colors
 * @param {string} params.config.colour_palette - Array of line colors (for leader lines)
        ...
        labelData.forEach(label => {
            ...
            if (useLeaderLines && Math.abs(label.y - label.originalY) > 1) {
                ...
                svg.append('line')
                    ...
                    .attr('y2', label.y)
                    .attr('stroke', config.colour_palette[label.categoryIndex % config.colour_palette.length])
                    .attr('stroke-width', 1)
