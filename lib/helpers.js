...
export function getXAxisTicks({ data, xDataType, size, config }) {
  ...
  if (xDataType === "date") {
    ...
    const domain = Array.from(new Set(data.map((d) => d.date)));
    ...
    if (method === "total") {
      ...
      if (count <= 1 || domain.length <= 1) {
        ticks = domain.slice(0, 1);
      } else if (count >= domain.length) {
        ticks = domain;
      } else {
        ticks = Array.from({ length: count }, (_, index) => {
          const position = Math.round(
            (index * (domain.length - 1)) / (count - 1)
          );
      ...
      ticks = domain.filter((_, index) => !(index % step));
    }

    if (!Array.isArray(ticks)) ticks = [];
    if (config.addFirstDate && !ticks.includes(domain[0])) {
      ticks.unshift(domain[0]);
    }
    if (
      config.addFinalDate &&
      !ticks.includes(domain[domain.length - 1])
    ) {
