...
const TIME_HIERARCHY = {
  ...
  quarter: {
    ...
    format: d => `Q${Math.floor(d.getUTCMonth() / 3) + 1}`,
    key: d => Math.floor(d.getUTCMonth() / 3),
    parent: "year"
  },
...
export function customTimeAxis(scaleInput) {
  let scale = scaleInput || null;
  let tickSize = 20;
  let tickPadding = 3;
  let userTickFormat = null;
  let userTickCount = null; // new
  let orient = "bottom"; // could extend later
  let minLabelWidth = 24; // sensible default; tweak per font
  let primaryUnitOverride = null; // "month" | "quarter" | "year" | null
  ...
  //     format: d => `${Math.floor(d.getUTCFullYear() / 10) * 10}s`,
  //     key: d => Math.floor(d.getUTCFullYear() / 10),
  //     parent: null
  //   }
  // };
  ...
  function filterByAvailableSpace(ranges, scale, minWidth) {
    return ranges.filter(r =>
      Math.abs(scale(r.end) - scale(r.start)) >= minWidth
    );
  }

  function primaryRanges(ticks, domainStart, domainEnd) {
    const ranges = [];

    if (ticks.length === 0) return ranges;

    // domain start → first tick
  ...
  function resolvePrimaryLevel(domainSpanMonths) {
    if (primaryUnitOverride) {
      const level = TIME_HIERARCHY[primaryUnitOverride];
      if (!level) {
        throw new Error(
          `Invalid primaryUnit "${primaryUnitOverride}". ` +
          `Valid values: ${Object.keys(TIME_HIERARCHY).join(", ")}`
        );
      }
      return level;
    }

    // Automatic selection (existing behavior)
    if (domainSpanMonths <= 12) return TIME_HIERARCHY.month;
    if (domainSpanMonths <= 36) return TIME_HIERARCHY.quarter;
    return TIME_HIERARCHY.year;
  }


  function parentRanges(ticks, parentLevel, domainStart, domainEnd) {
    const groups = d3.groups(ticks, d => parentLevel.key(d));

    return groups.map(([, group]) => {
      const start = d3.max([
        parentLevel.interval.floor(group[0]),
        domainStart
      ]);

      const end = d3.min([
        parentLevel.interval.offset(parentLevel.interval.floor(group[0]), 1),
        domainEnd
      ]);
  ...
  function isMajorTick(d, level) {
    if (!level.parent) return false;
    const parent = TIME_HIERARCHY[level.parent];
    return parent.interval.floor(d).getTime() === d.getTime();
  }

  function renderLabelLayer(g, scale, labels, format, offset) {
    const axisGen = (orient === "top" ? d3.axisTop : d3.axisBottom)(scale)
      .tickValues(labels)
      .tickSize(0)
  ...
  function axis(selection) {
    selection.each(function () {
      if (!scale) throw new Error("customTimeAxis requires a scale");

      const [start, end] = scale.domain();
      const domainSpanMonths =
        (end - start) / (1000 * 60 * 60 * 24 * 30.44);

      const primaryLevel = resolvePrimaryLevel(domainSpanMonths);
      const secondaryLevel =
        config?.labelSpans?.secondaryLabels && primaryLevel.parent
          ? TIME_HIERARCHY[primaryLevel.parent]
          : null;

      const ticks = userTickCount
        ? scale.ticks(userTickCount)
        : scale.ticks(primaryLevel.interval);
      ...
      // Tick marks only
      // -----------------------------
      const tickAxis = (orient === "top" ? d3.axisTop : d3.axisBottom)(scale)
        .tickValues(ticks)
        .tickSize(tickSize)
        .tickPadding(tickPadding)
        .tickFormat("");
      ...
      if (config.labelSpans.secondaryLabels) {
        g.selectAll(".tick line")
          .attr("y2", d =>
            isMajorTick(d, primaryLevel)
              ? tickSize * 1.5   // major tick
              : tickSize         // minor tick
          )
          .attr("stroke", d =>
            isMajorTick(d, primaryLevel)
          ...
          .attr("stroke-width", d =>
            isMajorTick(d, primaryLevel) ? 1.5 : 1
          );
      }

      const primaryRangesData = primaryRanges(ticks, start, end);

      const visiblePrimaryRanges = filterByAvailableSpace(
        primaryRangesData,
        scale,
        minLabelWidth
      );
      ...
      if (visiblePrimaryRanges.length) {
        renderLabelLayer(
          g,
          scale,
          visiblePrimaryRanges.map(d => d.mid),
          userTickFormat || primaryLevel.format,
          yOffset
        );
      ...
      if (secondaryLevel) {
        const parentRangesData = parentRanges(
          ticks,
          secondaryLevel,
          start,
          end
        ...
        const visibleParentRanges = filterByAvailableSpace(
          parentRangesData,
          scale,
          minLabelWidth * 1.2
        );

        if (visibleParentRanges.length) {
          const parentLayer = renderLabelLayer(
            g,
            scale,
            visibleParentRanges.map(d => d.mid),
            secondaryFormatOverride || secondaryLevel.format,
            yOffset
          );

          parentLayer.selectAll("text")
  ...
  axis.tickFormat = _ => arguments.length ? (userTickFormat = _, axis) : userTickFormat;
  axis.ticks = _ => arguments.length ? (userTickCount = +_, axis) : userTickCount;
  axis.orient = _ => arguments.length ? (orient = _, axis) : orient;
  axis.timeUnit = function (_) {
    return arguments.length
      ? ((primaryUnitOverride = _), axis)
      : primaryUnitOverride;
  };
...
export function customBandAxis(scaleInput) {
  let scale = scaleInput || null;
  let orient = "bottom";
  let tickSize = 6;
  let tickPadding = 3;
  let minLabelWidth = 16;
  ...
  const domainEnd = scale.domain()[scale.domain().length - 1];
  const paddingInnerPx = scale.step() - scale.bandwidth();

  // -----------------------------
  // Helpers
  // -----------------------------

  function isFullyWithinDomain(range, domainStart, domainEnd) {
    return (
      range.startDate >= domainStart &&
      range.endDate <= d3.utcDay.offset(domainEnd, 1)
    );
  }


  function bandCenter(d) {
    return scale(d) + scale.bandwidth() / 2;
  }

  function filterByAvailableSpace(ranges, minWidth) {
    return ranges.filter(r => r.width >= minWidth);
  }

  function parentRanges(domain, parentLevel) {
    ...
    return groups.map(([, values]) => {
      ...
      const end = scale(last) + scale.bandwidth();

      return {
        start: start - 0.5 * paddingInnerPx,
        end: end + 0.5 * paddingInnerPx,
        width: end - start,
  ...
  function renderLabelLayer(g, labels, format, offset) {
    const layer = g.append("g")
      .attr("class", "axis-label-layer")
      .attr("transform", `translate(0, ${offset})`);
    ...
    layer.selectAll("text")
      ...
      .attr("y", 0)
      .attr("text-anchor", "middle")
      .attr("dy", orient === "bottom" ? "0.71em" : "-0.25em")
      .text(d => format(d.date));

    return layer;
  }

  function isCompleteParentRange(range, parentLevel) {
    const expectedStart = parentLevel.interval.floor(range.date);
    const expectedEnd = parentLevel.interval.offset(expectedStart, 1);

    return (
      range.startDate.getTime() === expectedStart.getTime() &&
      range.endDate.getTime() === expectedEnd.getTime()
    );
  }


  function renderSecondaryBoundaries({
    g,
    ranges,
    parentLevel,
    height,
    ...
    // 1. keep only complete parent units
    const completeAndValidRangs = ranges.filter(r =>
      isCompleteParentRange(r, parentLevel) && isFullyWithinDomain(r, domainStart, domainEnd)
    );

    // 2. collect unique boundary positions
    const boundarySet = new Set();
    completeAndValidRangs.forEach(r => {
    ...
    g.append("g")
      .attr("class", "axis-secondary-boundaries")
      .selectAll("line")
      .data(boundaries)
      .join("line")
      .attr("x1", d => d)
  ...
  function axis(selection) {
    selection.each(function () {
      if (!scale) throw new Error("customBandAxis requires a scaleBand");
      ...
      const primaryLevel = TIME_HIERARCHY[primaryUnit];
      const secondaryLevel = primaryLevel.parent
        ? TIME_HIERARCHY[primaryLevel.parent]
        : null;
      ...
      g.append("g")
        ...
        .data(domain)
        .join("line")
        .attr("x1", d => bandCenter(d))
        .attr("x2", d => bandCenter(d))
        .attr("y1", 0)
      ...
      // Primary labels
      // -----------------------------
      const primaryRanges = domain.map(d => ({
        start: scale(d),
        end: scale(d) + scale.bandwidth(),
        width: scale.bandwidth(),
        mid: bandCenter(d),
        date: d
      }));

      const visiblePrimary = filterByAvailableSpace(
        primaryRanges,
        minLabelWidth
      );
      ...
      // -----------------------------
      // Secondary labels (parent unit)
      // -----------------------------
      if (secondaryLevel) {
        const parentRangesData = parentRanges(domain, secondaryLevel);

        const visibleParents = filterByAvailableSpace(
          parentRangesData,
          minLabelWidth * 2
        );

        if (visibleParents.length) {
          renderSecondaryBoundaries({
            g,
            ranges: visibleParents,
            parentLevel: secondaryLevel,
            height: yOffset + 14, // extend past primary labels
            orient
          });

          const layer = renderLabelLayer(
            g,
            visibleParents,
            secondaryFormatOverride || secondaryLevel.format,
            yOffset * sign
  ...
  axis.timeUnit = _ =>
    arguments.length ? (primaryUnit = _, axis) : primaryUnit;
